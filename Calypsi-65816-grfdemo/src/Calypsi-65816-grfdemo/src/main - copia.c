...
// Graphics Primitives
void clrBitmap();
void plot_4p_bezier (int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3, uint8_t col);
...
void setPixel (unsigned long x,unsigned  long y, uint8_t col); // Simple putPixel function Implementation for Vicky II 
int  getPixel (int x, int y);
void floodFill(int x, int y, int newColor);
void floodFill2(int x, int y, int newColor);
int  read_vram_buffer (uint8_t volatile __far *p, int length, int __far * buffer);
int  getScanLine (int y, int __far * buffer);
...
int main ()
{
  ...
  ANSIClrScr();
  /*
  printf("Press 0-3 to select Resolution\n\r");
  printf("0 - 640x480 pixels\n\r");
  printf("1 - 800x600 pixels\n\r");
  printf("2 - 320x240 pixels\n\r");
  printf("3 - 400x300 pixels\n\r");
  option=getchar(); // need to have getchar() implemented in Callypsi for the 65816 for this to work
  */
	//if (setupScreen((int)(option-48))==0)
	if (setupScreen(0)==0)
	{
		ANSIClrScr();
		srand(time(NULL)); 
	...
	/*
		printf("200 Lines...\n\r");
		 ...
		 for (c=0;c<=100;c++)
		  {
			...
			yRndEnd=rand() % (int)vickyResY;
			colRnd=rand() % 255;
			plot_4p_bezier (xRndInit, yRndInit, xRndMid, yRndMid,  xRndMid2, yRndMid2, xRndEnd, yRndEnd, colRnd);
		  } 
		  clrBitmap();
		  ANSIClrScr();           // Clear Text screen after switching resolution (for sanity!)
		  ANSILocate(1,1);
		  printf("100 Rectangles...\n\r");
		  for (c=0;c<=100;c++)
		  {
			xRndInit=rand() % (int)vickyResX;
			xRndEnd=rand() % (int)vickyResX;
		 ...
		 for (c=0;c<=50;c++)
		  ...
		  ANSILocate(1,1);
	*/
		  printf("50 Flood Filled Circles...\n\r");
		  for (c=0;c<=50;c++)
		  {
			xRndInit=rand() % (int)vickyResX;
			yRndInit=rand() % (int)vickyResY;
			...
			colRnd=rand() % 255;
			plot_circle (xRndInit, yRndInit, (yRndMid / 2),colRnd);
			floodFill2 (xRndInit, yRndInit,colRnd);
		  } 
		  // clrBitmap();
		  ANSIClrScr();           // Clear Text screen after switching resolution (for sanity!)
		  ANSILocate(1,1);
...
// -------------------------------------------------------------------
// Read VRAM Buffer Implementation by hth313 - Calypsi creator!
// Required for getPixel
// -------------------------------------------------------------------
 int read_vram_buffer (uint8_t volatile __far *p, int length, int __far *buffer) 
 {
...
// -----------------------------------------------------------------
// Just a simple delay to allow the FIFO to get completely populated
// -----------------------------------------------------------------
void FifoDelay()
{
	int loop;
	loop=0;
	while (loop++ < (int)vickyResX)
	;
}
...
// -----------------------------------------------------
// Simple getPixelLine Implementation by Ernesto Contreras
...
int getScanLine (int y, int __far *buffer) {
    ...
    if (vBitPlane[vickyBitmapPage]) // if Bitmap for the page is enabled
	{
		...
		if (y<vickyResY)
		{
			...
			ptr+= (long) bitplane[vickyBitmapPage].start;
			length = vickyResX-1; // read as many bytes as current vickyResX
			if (((long)ptr & 0xff0000)==((long)ptr+length & 0xff0000))
			// Test if bytes requested from FIFO cross a 64k Bank boundary
			{
				// of course they dont!
				st=0;
					st = read_vram_buffer(ptr, length+2, buffer);
			}
			else
			// Ooops this bytes just crossed the line!, let's do something about it! 
			// Let's do a FIFO request for each bank
			{
				printf ("Bank Line %d\r",y);
				...
				//printf("l2: %d l3:%d\r",length2,length3);
				// FIFO for 1st Bank
				st=0;
				...
				//while (st<vickyResX)
				//	{
				//		setPixel(st,y+1,(int)(buffer[st])+1);
				//		st++;
				//	}
				//exit (0);
			}
...
// -----------------------------------------------------
void setPixel (unsigned long x, unsigned long y, uint8_t col)
{
uint8_t __far24 *ptr;
	if (vBitPlane[vickyBitmapPage]) // if Bitmap for the page is enabled
		{
...
// removed boudary checking for speed optimization
// Warning: calling function is responsible that the x,y range is not out of bounds!
// ----------------------------------------------------------
...
// ----------------------------------------------------------
// clrBitmap function!  Ernesto Contreras, (modified memset function from Calypsi)
// ----------------------------------------------------------
void clrBitmap ()
{uint8_t __far24 *ptr;
...
// ----------------------------------------------------------------------------------
// Stack Functions (needed for Flood fill Implementation)
...
// Work in Progress for FloodFill

//------------------------------------------------------------------------------
// Scanline floodfill algorithm using stack instead of recursion, more robust
// floodFillScanlineStack(int x, int y, int newColor, int oldColor)
...
void floodFill(int x, int y, int newColor)
{
  ...
  while(stackPop(&x, &y))
  {
    ...
    while(x1 < vickyResX && getPixel(x1,y) == oldColor)
    {
      ...
      if(!spanBelow && y < vickyResY - 1 && getPixel(x1,y+1) == oldColor)
      {
        stackPush(x1, y + 1);
        spanBelow = 1;
      }
      else if(spanBelow && y < vickyResY - 1 && getPixel(x1,y+1)!= oldColor)
      {
        spanBelow = 0;
      }
      x1++;
...
// Global Array for Pixel buffer read
  ...
  static int BufferAbove[1024];
  static int BufferCurrent[1024];
  static int BufferBelow[1024];
